...
export default function LightToggleWidget({ widget, onDelete }: { widget: Widget; onDelete: () => void }) {
  ...
  const handleBrightness = async (value: number) => {
    setBrightness(value)
    if (!widget.entity_id || state !== 'on' || isAdjusting) return
    ...
    try {
      ...
      addToast(`Failed to set brightness for ${widget.title || widget.entity_id}`, 'error')
    } finally {
  ...
  return (
    <div className="bg-white p-3 sm:p-4 rounded-lg shadow hover:shadow-md transition border border-gray-100">
      ...
      <button
        ...
        className={`w-full py-2 px-3 rounded font-medium mb-2 sm:mb-3 transition text-sm sm:text-base ${
          state === 'on' ? 'bg-yellow-500 text-white hover:bg-yellow-600' : 'bg-gray-300 text-gray-700 hover:bg-gray-400'
        }`}
      >
        {isToggling ? (
      ...
      <p className="text-xs text-gray-500 truncate">{widget.entity_id}</p>
    </div>
  )
}
