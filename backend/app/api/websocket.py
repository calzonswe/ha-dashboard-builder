    ...
    try:
        ...
        while True:
            ...
            if data == "ping":
                ...
                manager.subscriptions[websocket].add(entity_id)
                logger.info(f"Client subscribed to {entity_id}")
                await websocket.send_json({"type": "subscribed", "entity_id": entity_id})
            elif data.startswith("unsubscribe:"):
                # Client can unsubscribe from specific entities
                entity_id = data.split(":")[1]
                manager.subscriptions[websocket].discard(entity_id)
                logger.info(f"Client unsubscribed from {entity_id}")
                await websocket.send_json({"type": "unsubscribed", "entity_id": entity_id})
            else:
